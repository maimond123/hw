...
nrow(airtemp)
airtemp |> count(year) |> arrange(desc(n)) |> head()
# Average the ice duration for both lakes 
icecover_avg <- icecover |>
  group_by(year) |>
  summarise(avg_ice_duration = mean(ice_duration)) |>
  ungroup()
  
data_new <- left_join(icecover_avg, airtemp, by = "year") 
...
The relationship look roughly linear and negative, since the spread of points around the slope line look fairly balanced, without strong curvature or other pattern. \
\
...
This means that only 11.6417% of the variation in the ice_duration is explained by the average air temperature.
...
The RMSE is about 18 days, meaning that on average our predictions for ice duration are off by nearly three weeks. From the the ice_duration vs. air_temp plot, we could see that typical ice durations range from 80 to 140 days, so the prediction error is fairly large in context, suggesting that while air temperature is related to ice duration, it is not the only factor.
...
a\. The slope of -6.182 indicates that, on average, for every one degree celsius increase in average air temperate, the average_ice_duration for the 2 lakes decreases by 6.182 days. \

```{r}
tail(ice_duration_avg)
```
...
\
The residual is calculated as (e = Y - Y-hat). This means e = (
